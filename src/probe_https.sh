...
probe_http() {
  ...
  if [[ "$scheme" == "https" ]]; then
    ...
    out="$(curl -sS -k -o /dev/null --connect-timeout 2 --max-time 5 -w "%{http_code} %{time_total}" "$url")" || {
      echo "ERR,NA"; return 0;
    }
  else
    out="$(curl -sS -o /dev/null --connect-timeout 2 --max-time 5 -w "%{http_code} %{time_total}" "$url")" || {
      echo "ERR,NA"; return 0;
    }
  fi
  ...
  local time_ms
  time_ms="$(awk -v t="$time_s" 'BEGIN{ printf "%06d", int(t*1000 + 0.5) }')"
